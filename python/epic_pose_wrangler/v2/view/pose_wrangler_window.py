...
class PoseWranglerWindow(MayaQWidgetDockableMixin, QtWidgets.QMainWindow):
    ...
    event_import_drivers = QtCore.Signal(str, list)
    ...
    event_set_skeleton_config = QtCore.Signal(str)
    ...
    def __init__(self, version="2.0.0"):
        ...
        self._resource_folder = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'resources')
        icon_folder = self._resource_folder + "/icons"
        QtCore.QDir.setSearchPaths("PoseWrangler", [icon_folder])
        self.setWindowTitle("Pose Wrangler {version}".format(version=version))
        ...
        # Utility Connections
        self.win.skeleton_config_BTN.clicked.connect(self.set_mirror_file)
    ...
    def edit_blendshape(self, pose_name, edit=False):
        ...
        """
        # Iterate through all the driven transforms in the list
    ...
    def edit_solver(self, solver, edit=False):
        ...
        if edit:
            ...
            self._editing_solver = None
            # Revert the button text
            self.win.toggle_edit_BTN.setText("Edit Selected Solver")
            # Clear the edit icon
    ...
    def set_mirror_file(self):
        ...
        result = QtWidgets.QFileDialog.getOpenFileName(
            self, "Skeleton Config File",
            os.path.join(os.path.dirname(__file__), 'configs'),
            "Skeleton Config File (*.json)"
        )
        # If no file specified, exit early
        if not result[0]:
            return
        # Emit the event
        self.event_set_skeleton_config.emit(result[0])

    def update_skeleton_config_file(self, path):
        """
        Update the mirror mapping widget with the specified path
        :param path :type str: file path to the mirror mapping file
        """
        # Set the text
        self.win.skeleton_config_LINE.setText(path)
    ...
    def _open_documentation(self):
        ...
        index_html = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '../../..', 'docs', 'site', 'index.html')
        )
    ...
    def _set_stylesheet(self):
        ...
        if bool(int(settings.SettingsManager.get_setting("UseMayaStyle") or 0)):
            styles_path = os.path.join(self._resource_folder, 'style', 'maya_style.qss')
        else:
            styles_path = os.path.join(self._resource_folder, 'style', 'style.qss')
