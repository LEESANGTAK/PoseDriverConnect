...
class UEPoseBlenderNode(object):
    ...
    def create(cls, driven_transform=None):
        ...
        if driven_transform is not None:
            ...
            # Set the base pose
            node_ref.base_pose = cls._get_local_matrix_without_joint_orient(transform_name=driven_transform)
    ...
    def add_pose_from_current(self, pose_name, index=-1):
        ...
        self.set_pose(
            index=index,
            pose_name=pose_name,
            matrix=self._get_local_matrix_without_joint_orient(
    ...
    def set_pose(self, index=-1, pose_name="", overwrite=True, matrix=None):
        ...
        if matrix is None:
            matrix = self._get_local_matrix_without_joint_orient(transform_name=self.driven_transform)
    ...
    @classmethod
    def _get_local_matrix_without_joint_orient(cls, transform_name):
        """
        Remove joint orient values from the driven transforms matrix if the driven transform is a joint
        """
        if cmds.objectType(transform_name) == 'joint':
            inverse_joint_orient_matrix = om.MMatrix(utils.compose_matrix(
                position=(0, 0, 0),
            ...
            object_space_matrix = om.MMatrix(cmds.xform(transform_name, query=True, matrix=True, objectSpace=True))

            m = object_space_matrix * inverse_joint_orient_matrix
            _, rotation, _ = utils.decompose_matrix(m)
            translation, _, scale = utils.decompose_matrix(object_space_matrix)

            return list(utils.compose_matrix(translation, rotation, scale))
